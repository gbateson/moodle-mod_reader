...
//Returns every needed user (participant) in a course
     ...
     * @uses $CFG
     * @param xxx $courseid
     * @param xxx $includemessages (optional, default=false)
     * @param xxx $includeblogs (optional, default=false)
     * @return xxx
...
//Returns every enrolled user (student and teacher) in a course
    ...
    function backup_get_enrolled_users($courseid) {
        ...
        return get_users_by_capability(reader_get_context(CONTEXT_COURSE, $courseid), 'moodle/course:view');
    }
...
//Return the xml start tag
     ...
     * @param xxx $endline (optional, default=false)
     * @param xxx $attributes (optional, default=null)
     * @return xxx
     * @todo Finish documenting this function
     */
...
//Return the xml end tag
     ...
     * @param xxx $tag
     * @param xxx $level (optional, default=0)
     * @param xxx $endline (optional, default=true)
     * @return xxx
     * @todo Finish documenting this function
     */
...
//Prints General info about the course
    ...
    function backup_general_info($bf,$preferences) {
        ...
        $coursecontext = reader_get_context(CONTEXT_COURSE, $preferences->backup_course);
...
//Prints course's general info (table course)
    ...
    function backup_course_start($bf,$preferences) {
        ...
        if ($course) {
            ...
            fwrite ($bf,full_tag("NUMSECTIONS",3,false,reader_get_numsections($course)));
            ...
            if ($instances = blocks_get_by_page($page)) {
                ...
                foreach ($instances as $position) {
                    foreach ($position as $instance) {
                        ...
                        $context = reader_get_context(CONTEXT_BLOCK, $instance->id);
...
//Prints course's modules info (table course_modules)
    ...
    function backup_course_modules($bf,$preferences,$section) {
        ...
        while ($tok) {
           ...
           if ($selected) {
               $context = reader_get_context(CONTEXT_MODULE, $tok);
    ...
    function backup_user_info($bf,$preferences) {
        ...
        if ($users && !rs_EOF($users)) {
            ...
            while ($user = rs_fetch_next_record($users)) {
                ...
                $context = reader_get_context(CONTEXT_USER, $user->old_id);
...
//Backup gradebook_item (called from backup_gradebook_info
    ...
    function backup_gradebook_grade_letters_info($bf, $preferences) {
        ...
        $context = reader_get_context(CONTEXT_COURSE, $preferences->backup_course);
...
//This function copies the final zip to the course dir
    ...
    function backup_fetch_roles($preferences) {
        ...
        if ($courseblocks = get_records_sql("SELECT *
            ...
            foreach ($courseblocks as $courseblock) {
                ...
                $context = reader_get_context(CONTEXT_BLOCK, $courseblock->id);
                $contexts[$context->id] = $context;
            }
        }
     ...
     *
     * @uses $CFG
     * @param xxx $preferences (passed by reference)
