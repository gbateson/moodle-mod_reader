...
//This function create a new course record.
     ...
     * @uses $DB
     * @param xxx $restore
     * @param xxx $course_header (passed by reference)
     * @return xxx
     * @todo Finish documenting this function
     */
    function restore_create_new_course($restore,&$course_header) {
        ...
        do {
            ...
            $coursefull  = $DB->get_record('course',array('fullname' => addslashes($currentfullname)));
            $courseshort = $DB->get_record('course',array('shortname' => addslashes($currentshortname)));
            $counter++;
        ...
        if (! $category) {
            $category = $DB->get_record('course_categories',array('name' => addslashes($course_header->category->name)));
        }
...
//This function creates all the course_sections and course_modules from xml
     ...
     * @uses $DB
     * @uses $db
     * @param xxx $restore (passed by reference)
     * @param xxx $xml_file
     * @return xxx
     * @todo Finish documenting this function
     */
            ...
            if ($recs) {
                foreach ($recs as $rec) {
                    ...
                    if ($data) {
                        ...
                        if (has_capability('moodle/grade:manageoutcomes', reader_get_context(CONTEXT_SYSTEM))) {
...
//This function creates all the user, user_students, user_teachers
    ...
    function restore_create_users($restore,$xml_file) {
        ...
        /// Grab mnethosts keyed by wwwroot, to map to id
            $mnethosts = $DB->get_records('mnet_host', array(), 'wwwroot, id');
        ...
        /// Init trailing messages
            ...
            foreach ($info->users as $userid) {
                ...
                $user_exists = true;
                $user_data = $DB->get_record('user',array('username' => addslashes($user->username),
            ...
            if ($scales !== true) {
                ...
                foreach ($scales as $scale) {
                    ...
                    if ($data) {
                        ...
                        $sca_db = $DB->get_record_sql('SELECT * FROM {scale}
                                                           WHERE scale = ?
                                                           AND courseid = ?', array($sca->scale,$course_to_search));
        ...
        foreach ($groups as $group) {
            ...
            if ($data) {
                ...
                if (! $gro_db = $DB->get_record_sql('SELECT *
                                          FROM {groups}
                                          WHERE courseid = ? AND
                                                name = ?' . $description_clause, array($restore->course_id,$gro->name))) {
                    //If it doesn't exist, create
    ...
    function restore_create_groupings($restore,$xml_file) {
        ...
        foreach ($groupings as $grouping) {
            if ($data = backup_getid($restore->backup_unique_code,"groupings",$grouping->id)) {
                ...
                //Now search if that group exists (by name and description field) in
                if ($gro_db = $DB->get_record('groupings', 'courseid', $restore->course_id, 'name', $gro->name, 'description', $gro->description)) {
                    //get current group id
                    $newid = $gro_db->id;
    ...
    function restore_check_instances($restore) {
        ...
        if ($course_modules) {
            foreach($course_modules as $cm) {
               ...
               $course_modules_inst_zero = $DB->get_records_sql("SELECT id, course, instance
                                           FROM {course_modules}
                                           WHERE id = ? AND
                                                 instance = '0'", array($cm_module->new_id));
...
//This function executes the MoodleParser
     ...
     *
     * @param xxx $backup
     * @param xxx $k (optional, default=0)
     * @return xxx
     * @todo Finish documenting this function
     */
     ...
     * @param xxx $info
     * @param xxx $course_header
     * @param xxx $errorstr (passed by reference)
     * @return xxx
     * @todo Finish documenting this function
     */
    function restore_execute(&$restore,$info,$course_header,&$errorstr) {
        ...
        if ($status) {
            ...
            // fix for MDL-6831
            $newcontext = reader_get_context(CONTEXT_COURSE, $restore->course_id);
            if (! has_capability('moodle/course:manageactivities', $newcontext)) {
                ...
                if ($CFG->creatornewroleid) {
                    ...
                    if ($legacyteachers = get_roles_with_capability('moodle/legacy:editingteacher', CAP_ALLOW, reader_get_context(CONTEXT_SYSTEM))) {
...
/********************** Roles and Capabilities Related Functions *******************************/
    ...
    function restore_create_roles($restore, $xmlfile) {
        ...
        $sitecontext = reader_get_context(CONTEXT_SYSTEM);
        ...
        if (isset($info->roles) && $info->roles) {
            ...
            /// Now, restore role nameincourse (only if the role had nameincourse in backup)
                if (! empty($roledata->nameincourse)) {
                    ...
                    $coursecontext = reader_get_context(CONTEXT_COURSE, $restore->course_id); /// Look for target context
...
// auxillary function to write role assignments read from xml to db
     ...
     *
     * @param xxx $restore
     * @param xxx $assignments
     * @param xxx $table
     * @param xxx $contextlevel
     * @param xxx $oldid
     * @param xxx $oldroleid
     * @todo Finish documenting this function
     */
    function restore_write_roleassignments($restore, $assignments, $table, $contextlevel, $oldid, $oldroleid) {
        ...
        foreach ($assignments as $assignment) {
            ...
            $newcontext = reader_get_context($contextlevel, $oldinstance->new_id);
...
//write activity date changes to the html log file, and update date values in the the xml array
     ...
     *
     * @uses $CFG
     * @param xxx $recordtype
     * @param xxx $restore (passed by reference)
     * @param xxx $xml (passed by reference)
     * @param xxx $TAGS
     * @param xxx $NAMETAG (optional, default='NAME')
     * @todo Finish documenting this function
     */
