...
class back_match_qtype {
    ...
    function get_question_options(&$question) {
        ...
        $question->options->subquestions = $DB->get_records('question_match_sub', array('question'=> $question->id));
        return true;
    }
    ...
    function save_question_options($question) {
        ...
        if (! $oldsubquestions = $DB->get_records('question_match_sub', array('question' => $question->id))) {
        ...
        foreach ($question->subquestions as $key => $questiontext) {
            ...
            if ($questiontext != '' || $answertext != '') {
                if ($subquestion = array_shift($oldsubquestions)) {  // Existing answer, so reuse it
                    ...
                    $subquestion->questiontext = stripslashes($questiontext);
                    $subquestion->answertext   = stripslashes($answertext);
    ...
    */
     ...
     * @uses $DB
     * @param xxx $question (passed by reference)
     * @param xxx $state (passed by reference)
     * @param xxx $cmoptions
     * @param xxx $attempt
     * @return xxx
     * @todo Finish documenting this function
    ...
    function create_session_and_responses(&$question, &$state, $cmoptions, $attempt) {
        global $DB;
        if (! $state->options->subquestions = $DB->get_records('question_match_sub', array('question'=> $question->id))) {
    ...
    function restore_session_and_responses(&$question, &$state) {
        ...
        if (! $questions = $DB->get_records('question_match_sub', array('question'=> $question->id))) {
           notify('Error: Missing subquestions!');
           return false;
        }
...
// ULPGC ecastro
     ...
     * @param xxx $question
     * @param xxx $state
     * @param xxx $length (optional, default=80)
     * @return xxx
     * @todo Finish documenting this function
     */
    ...
    function restore($old_question_id,$new_question_id,$info,$restore) {
        ...
        //The structure is equal to the db, so insert the question_match_sub
        if (empty($match->correctfeedback))
          $match->correctfeedback = "";
        if (empty($match->partiallycorrectfeedback))
          $match->partiallycorrectfeedback = "";
        if (empty($match->incorrectfeedback))
     ...
     *
     * @param xxx $question
     * @param xxx $fromcourseid
     * @param xxx $tocourseid
     * @param xxx $url
     * @param xxx $destination
     * @todo Finish documenting this function
     */
