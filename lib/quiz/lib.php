...
function quiz_get_user_attempts($quizid, $userid, $status = 'finished', $includepreviews = false) {
    ...
    if ($attempts = $DB->get_records_select('quiz_attempts', $select, $params, 'attempt ASC')) {
        return $attempts;
    } else {
        return array();
    }
...
*/
    $is_closed = ($quiz->review & QUIZ_REVIEW_SCORES & QUIZ_REVIEW_CLOSED);
    $is_open   = ($quiz->review & QUIZ_REVIEW_SCORES & QUIZ_REVIEW_OPEN);
 ...
 * quiz_refresh_events
 *
 * @param xxx $courseid (optional, default=0)
 * @return xxx
 * @todo Finish documenting this function
 */
...
function quiz_get_recent_mod_activity(&$activities, &$index, $timestart, $courseid, $cmid, $userid=0, $groupid=0)  {
    ...
    $cm_context      = reader_get_context(CONTEXT_MODULE, $cm->id);
...
function quiz_print_recent_mod_activity($activity, $courseid, $detail, $modnames) {
    ...
    if ($detail) {
        ...
        echo "<img src=\"$CFG->modpixpath/{$activity->type}/pix/icon.gif\" ".
...
function quiz_check_file_access($attemptuniqueid, $questionid) {
    ...
    $context = reader_get_context(CONTEXT_COURSE, $quiz->course);
...
/// This is the same as what the lesson does, and the dabate is in MDL-10568.
    ...
    foreach ($quizzes as $quiz) {
        ...
        /// Now provide more information depending on the uers's role.
            $context = reader_get_context(CONTEXT_MODULE, $quiz->coursemodule);
