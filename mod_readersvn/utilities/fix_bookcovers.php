...
/**
 * mod/reader/utilities/fix_bookcovers.php
 *
 * @package    mod
 * @subpackage reader
...
function reader_get_itemids($readercfg) {
    ...
    if (! $xml = reader_curlfile($xml_file)) {
        echo 'Oops - no images found';
        return false;
    }

    if (! $xml = xmlize(implode('', $xml))) {
        echo 'Oops - could not create xml for images';
        return false;
    }

    if (! isset($xml['myxml']['#']['item'])) {
        echo "Oops - no 'item' tag in images xml";
        return false;
    }
...
function reader_print_images_form($readercfg, $action) {
    ...
    $params = array('method' => 'post', 'action' => $CFG->wwwroot.'/mod/reader/utilities/fix_bookcovers.php');
    ...
    $params = array('type' => 'radio', 'name' => 'action', 'value' => 'all');
    if ($action=='all') {
        $params['checked'] = 'checked';
    }
    echo html_writer::empty_tag('input', $params).' ';
    echo get_string('allbooks', 'reader');
    echo html_writer::empty_tag('br');

    $params = array('type' => 'radio', 'name' => 'action', 'value' => 'my');
    if ($action=='my') {
        $params['checked'] = 'checked';
    }
    echo html_writer::empty_tag('input', $params).' ';
    echo get_string('mybooks', 'reader');
    echo html_writer::empty_tag('br');
...
function reader_print_all_done() {
    ...
    $params = array('href' => new moodle_url('/mod/reader/utilities/index.php'));
    echo html_writer::tag('p', html_writer::tag('a', get_string('clicktocontinue', 'reader'), $params));
}
