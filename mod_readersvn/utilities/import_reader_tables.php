...
// set title
$title = get_string('importreadertables', 'reader');
...
if ($books = $DB->get_records($booktable)) {
    ...
    foreach ($books as $book) {
        ...
        if ($readercourseid==0) {
            ...
            $readercourseid = $DB->get_field('course', 'id', array('shortname' => $coursename));
...
// create users used in "reader_attempts" and "reader_deleted_attempts"
...
// create groups used in "reader_goal"
if ($goals = $DB->get_records('reader_goal', null, 'readerid')) {
...
if ($messages = $DB->get_records('reader_messages', null, 'instance,teacherid')) {
    ...
    foreach ($messages as $message) {
        ...
        $firsttime = true;
        if ($reader && $reader->id==$message->instance) {
            // same $reader, $course, $coursecontext, $enrol
        } else if (! $reader = $DB->get_record('reader', array('id' => $message->instance))) {
            continue; // shoudn't happen !!
        } else if (! $course = $DB->get_record('course', array('id' => $reader->course))) {
            continue; // shoudn't happen !!
        } else if (! $coursecontext = reader_get_context(CONTEXT_COURSE, $course->id)) {
...
// return to Reader utilities index page
$href = new moodle_url('/mod/reader/utilities/index.php');
...
function print_import_tables_form($deletecourses, $deletecategories) {
    ...
    // start form
    $params = array('method' => 'post', 'action' => $CFG->wwwroot.'/mod/reader/utilities/import_reader_tables.php');
